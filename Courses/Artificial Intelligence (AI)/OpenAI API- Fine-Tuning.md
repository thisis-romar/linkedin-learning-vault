...
> [!info]- Semantic Content
>
> **Frameworks & Libraries:** [JSON](../../Skills/Web%20Development/JSON.md) (12), [Python](../../Skills/Software%20Development/Python%20(Programming%20Language).md) (7), [OpenAI API](../../Skills/Artificial%20Intelligence%20(AI)/OpenAI%20API.md) (2), [GitHub](../../Skills/Software%20Development/GitHub.md) (1), [Codespaces](../../Skills/Software%20Development/Codespaces.md) (1)
...
> [!info]- Semantic Content
>
> **Frameworks & Libraries:** [Tokens](../../Skills/Web%20Development/Tokens.md) (5), [JSON](../../Skills/Web%20Development/JSON.md) (1), [Fine Tuning](../../Skills/Software%20Development/Fine%20Tuning.md) (1)
...
> [!info]- Semantic Content
>
> **Frameworks & Libraries:** [Fine Tuning](../../Skills/Software%20Development/Fine%20Tuning.md) (8), [GPT-4](../../Skills/Artificial%20Intelligence%20(AI)/GPT-4.md) (4), [Tokens](../../Skills/Web%20Development/Tokens.md) (2), [Java](../../Skills/Software%20Development/Java.md) (1)
...
> [!transcript]- Transcript
>
> **[0:00](https://www.linkedin.com/learning/openai-api-fine-tuning-21058733/uploading-training-data-to-the-api?u=76281980&t=0)** - [Instructor] The first step in the fine-tuning process is to generate the training and validation files. I already covered this in the previous chapter, and I have the files sitting here in [Codespaces](../../Skills/Software%20Development/Codespaces.md) under the data folder. If you don't have these files, or you started on this chapter, and didn't watch the previous one, go back to the previous chapter and watch video number two, where I break down how to create these training files. Now we're going to upload these files using the [OpenAI API](../../Skills/Artificial%20Intelligence%20(AI)/OpenAI%20API.md), and to do that, you need the OpenAI API key. In the exercise files video at the very top of the course, I explained how to set up this key. You can see I've already set it up. Again, if you haven't done this step, go back and watch the second video of this course, where I explain how to complete this step. Once you have the.env file with the API key, you're ready to move to the second step. Fine-tune-flow.ipynb is a Jupyter notebook, and to start the process, we need to import all the packages that the code depends on. So I'll go down to this first code block, and I can either hit shift return, or I can click on this play button here to play out the code. The first time you do this, you always have to select an environment for the code to run in, so I'm going to select [Python](../../Skills/Software%20Development/Python%20(Programming%20Language).md). Then the code runs, and it'll take a little bit of time on this first code block because the Python environment has to be configured and set up. Once the timer down here in the corner is completed, you know that all the packages have been imported and everything is working properly. Next, we set up the data. Here we're creating variable names for the different data files. So, I'll hit shift return to run that block. Then we get to the first step of this process,
>
> **[1:32](https://www.linkedin.com/learning/openai-api-fine-tuning-21058733/uploading-training-data-to-the-api?u=76281980&t=92)** which is to upload the files. Here, I'm using the client files create method to upload the files into the API. The same methods are also provided in the OpenAI node library if you prefer [JavaScript](../../Skills/Software%20Development/JavaScript.md). In this code, I'm opening each of the files, so training and validation, and then using the client files create method to pass the files in. And I append this attribute, purpose fine tune, to tell the system I'm going to use these for [Fine Tuning](../../Skills/Software%20Development/Fine%20Tuning.md). I'll run this code block, scroll up so you can see the response. And here I get the training file ID and validation file ID. These are the IDs for the files that now exist inside the OpenAI storage system. And I'm going to refer back to these IDs anytime I want to refer back to the files. Now scrolling up into the code, you'll see I've saved these IDs as training file ID, and validation file ID, so we can use them later on in the code without having to copy and paste it in anywhere.

> [!info]- Semantic Content
>
> **Frameworks & Libraries:** [OpenAI API](../../Skills/Artificial%20Intelligence%20(AI)/OpenAI%20API.md) (2), [Python](../../Skills/Software%20Development/Python%20(Programming%20Language).md) (2), [Codespaces](../../Skills/Software%20Development/Codespaces.md) (1), [JavaScript](../../Skills/Software%20Development/JavaScript.md) (1), [Fine Tuning](../../Skills/Software%20Development/Fine%20Tuning.md) (1)
> **Env Vars:** api (4)
...
> [!info]- Semantic Content
>
> **Frameworks & Libraries:** [Fine Tuning](../../Skills/Software%20Development/Fine%20Tuning.md) (4)
> **Env Vars:** api (4)
...
> [!info]- Semantic Content
>
> **Frameworks & Libraries:** [Fine Tuning](../../Skills/Software%20Development/Fine%20Tuning.md) (8), [Tokens](../../Skills/Web%20Development/Tokens.md) (2), [OpenAI API](../../Skills/Artificial%20Intelligence%20(AI)/OpenAI%20API.md) (1)
> **Env Vars:** api (3)
> **CLI Commands:** make (1), find (1)
...
> [!transcript]- Transcript
>
> **[0:00](https://www.linkedin.com/learning/openai-api-fine-tuning-21058733/using-the-fine-tuned-model-through-the-api?u=76281980&t=0)** - [Instructor] With the new model name in hand, it's time to test the model. And if you've ever used the [OpenAI API](../../Skills/Artificial%20Intelligence%20(AI)/OpenAI%20API.md) before, we're now firmly back in familiar territory. Here we're going to use the client.chat.completions.create() method, which is the standard create method you use anytime you want to create a chat with OpenAI. And the only difference between this request and request you've done before is in place of putting in the regular model name, so [ChatGPT](../../Skills/Artificial%20Intelligence%20(AI)/ChatGPT.md)-3.5 Turbo, you put in this giant new fine tuned model name. Everything else stays the same as evidenced down here. Test messages, we pass in a system message and a user message. And you'll notice here that the system message matches the system message we had in the training data. So if you've lost that system message, you can go back here and look at the training data. Here we have the system message. Just copy that system message out, and place it in as a system message. And then, we're passing in a restaurant review. I send all this information to client.chat.completions.create, passing in the model name for the fine tuned model, the messages, setting the temperature to zero, and setting the max [Tokens](../../Skills/Web%20Development/Tokens.md) to 500. I specifically set the temperature to zero because I don't want a lot of variance here because I want consistent output. By the way, if you want to learn more about how to make nondeterministic APIs more deterministic, I have an entire course for you in the library. Just search for nondeterministic APIs. Anyway, let's pass this request onto the API and see what happens. The request completes and out I get the [JSON](../../Skills/Web%20Development/JSON.md) object
>
...
> [!info]- Semantic Content
>
> **Frameworks & Libraries:** [OpenAI API](../../Skills/Artificial%20Intelligence%20(AI)/OpenAI%20API.md) (1), [ChatGPT](../../Skills/Artificial%20Intelligence%20(AI)/ChatGPT.md) (1), [Tokens](../../Skills/Web%20Development/Tokens.md) (1), [JSON](../../Skills/Web%20Development/JSON.md) (1)
> **Env Vars:** api (3), json (1)
> **CLI Commands:** make (1)
> **Versions:** 3.5 (1)
> **Speakers:** - [instructor] (1)
...
← [OpenAI API- Embeddings](OpenAI%20API-%20Embeddings.md) | **5 of 10** | [OpenAI API- Image Generation with DALL-E](OpenAI%20API-%20Image%20Generation%20with%20DALL-E.md) →
