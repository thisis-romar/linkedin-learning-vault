...
> [!transcript]- Transcript
>
> **[0:01](https://www.linkedin.com/learning/statistics-foundations-1-the-basics/introduction-to-variability?u=76281980&t=1)** - Mean, median, and mode, all helpful in understanding a data set. They're all simple concepts and all easy to calculate, but they have their limitations. For example, take these two dots or data points. Where is their midpoint? It's here. How about these two data points? Same middle. Both sets of dots have the same midpoint, but clearly the two sets of dots are very different. When we use real numbers, we find the same thing. All three of these tiny data sets have the same mean. The mean is 30, but each data set is obviously different. In data set one, the two data points are very close, but by the time we get to data set three, the two data points are very far apart. So how do we describe how close or how far away data points are from each other in a data set? In this chapter, we'll discuss range and standard deviation, two measures that help us understand how spread out the data points are in our data sets, which is something that statisticians like to refer to as variability, plus we'll discuss the very important empirical rule. And finally, we'll try and figure out what constitutes an outlier.
...
> [!transcript]- Transcript
>
> **[0:00](https://www.linkedin.com/learning/statistics-foundations-1-the-basics/range?u=76281980&t=0)** - Look at these two small data sets. Both have the same average and median values. So, both have the same middle. Other than that, the data sets are quite different. These data sets are small, so we can quickly view all the data and see the differences, but what happens when the data sets are enormous? How could you measure variability? Well, if we think back to mean and median, we looked at the middle of the data set. So why not look at the edges of the data set? This is what the range does. The range is one of our measures of variability. To find the range, you simply subtract the smallest data point from the largest data point in the data set. So when we look at this data set, our range is 50, but in this data set, our range is 150. Easy enough, so let's put range to work. We can use this data set to explore how mean, median, and range can work together. 15 students took an exam. Here are the mean, median, and range: mean 60, median 58, range 70. Some may jump to the conclusion that the maximum value is 95 and the minimum value is 25. But here's the data.
>
> **[1:38](https://www.linkedin.com/learning/statistics-foundations-1-the-basics/range?u=76281980&t=98)** The minimum data point is 10. The maximum data point is 80. And if we use a histogram, we understand the data set even better. It's really because of one student the range is so large. Their lack of studying inflated the range. The range is a simple measure of variability. But as we just saw, it only takes one rogue data point to exaggerate the size of your data set's range, and that's why we'll want to look at standard deviation. The range only takes into account two data points. The standard deviation, on the other hand, measures variability by considering every data point. Let's look at that next.
...
> [!transcript]- Transcript
>
> **[0:00](https://www.linkedin.com/learning/statistics-foundations-1-the-basics/calculating-standard-deviation?u=76281980&t=0)** - [Narrator] While a spreadsheet can easily calculate the standard deviation of a given dataset, let's dig in and calculate the standard deviation ourselves. A reminder, standard deviation is sort of the average distance from the data set's mean to all the individual data points, but not quite. It's actually the average squared distance from the mean. Here's the formula for standard deviation. As you might imagine, the Sigma warns us that we'll be doing this calculation for every data point in our dataset. Yes, these calculations could get ugly quick. So to keep things simple, we'll be using a very small dataset. Let's use this data set with only four values, two, eight, 10, and 12. First, we need to find the mean of these four data points, which we'll call X-bar. The average of these four data points is eight. So X-bar is equal to eight. We also need N, the sample size. We have four data points, so N is equal to four. So now our formula looks like this, eight is our X-bar, four is our N. And remember, the Greek letter Sigma tells us we need to do the X minus eight calculation for each of our four data points. So for our first data point, we take two minus the mean of eight, which gives us negative six and negative six squared is 36. Our second data point is eight,
>
> **[1:34](https://www.linkedin.com/learning/statistics-foundations-1-the-basics/calculating-standard-deviation?u=76281980&t=94)** eight minus eight means we square zero. Our third data point is 10 minus eight, which is two. So we square two, and our final data point is 12, 12 minus eight gives us four, so we square four. We then add up all the values in our numerator, 36, zero, four and 16, which gives us 56, which means we have 56 divided by three. This value, 18.7, we call this our variance, but the square root sign tells us we aren't done. And if we take the square root of 18.7, our variance, we get 4.32, which means the standard deviation of our dataset is 4.32. This was a small dataset. So calculating the standard deviation step-by-step was manageable, but as your data sets get really large, you may want to utilize a spreadsheet to help you find your standard deviation.
...
> [!transcript]- Transcript
>
> **[0:01](https://www.linkedin.com/learning/statistics-foundations-1-the-basics/empirical-rule?u=76281980&t=1)** - The empirical rule. I know, it sounds scary. But, actually the empirical rule is very useful for understanding the distribution of data points in our data sets. So what is it? First, it's important to note that this rule works for symmetrically, or normally distributed data. On this chart, the zero is in the middle of the x-axis. That's the mean of the data set. The data points in our data set are distributed in a bell shaped pattern that is centered at the mean. As you can see, our data are symmetrically distributed, or mirrored on each side of the y-axis. We call this a normal distribution. Once we understand this, we can now break down the empirical rule. The empirical rule tells us that most of the data points in our data set fall within three standard deviations of the mean. Notice, at the bottom along the x-axis the numbers one, two, three moving to the right and negative one, negative two, and negative three, moving to the left. The distance between each number represents one standard deviation. What's so interesting about the empirical rule is that it says when you have symmetrical distribution of the data, you can expect 68% of all your data points
>
> **[1:37](https://www.linkedin.com/learning/statistics-foundations-1-the-basics/empirical-rule?u=76281980&t=97)** to be within one standard deviation. So, 68% of all the data points are within negative one and positive one on this graph. The empirical rule then goes on to say that 95% of all data points fall within two standard deviations of the mean. Again on our graph, 95% of all the data points are expected to be within negative two and positive two on this graph. And finally, the empirical rule tells us that when you have the bell shaped curve, often referred to as a normal distribution, 99.7% of the data points in the dataset will fall within three standard deviations. So, on our graph, 99.7% of all of our data points in the data set are under the curve between negative three and positive three. Only 0.3% of all our data is expected to fall outside of that region. A reminder though, this works when we have the well-centered, symmetrical bell shaped curve. Unfortunately, the empirical rule begins to lose value the farther our data set strays from the classical, normal distribution. To review, if a data point has a z-score of negative 0.8 the data point is to the left of the mean
>
> **[3:14](https://www.linkedin.com/learning/statistics-foundations-1-the-basics/empirical-rule?u=76281980&t=194)** near the negative 0.1. It's within one standard deviation of the mean, and it is part of the 68% of the data points that are closest to the mean. But, if a data point had a z-score above 3.2, we call that particular data point an outlier. Because, it's probably not among the 99.7% of all the data points closest to the mean. It would be very far to the right, a bit beyond three. As you can see, the empirical rule, the bell curve, and z-scores are great tools in understanding our data set as a whole, as well as, the individual data points in our data set.
