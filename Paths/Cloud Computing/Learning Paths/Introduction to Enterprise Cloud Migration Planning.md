---
type: learning-path
cssclasses:
  - lle-learning-path
url: https://www.linkedin.com/learning/paths/introduction-to-enterprise-cloud-migration-planning
course_count: 4
linkedin_topics:
  - Cloud Computing
  - DevOps
  - Software Development
louvain_cluster: 3
tags:
  - learning-path
  - topic/cloud-computing
  - topic/devops
  - topic/software-development
  - status/fresh
last_activity: null
activity_total: 0
courses_visited: 0
videos_visited: 0
progress_status: fresh
course_activity:
  - course: '[[Cloud Concepts Planning An Application Migration Strategy]]'
    last_opened: null
    open_count: 0
    videos: []
  - course: '[[Design a Cloud Migration Strategy]]'
    last_opened: null
    open_count: 0
    videos: []
  - course: '[[Dependency Mapping for Cloud Migration]]'
    last_opened: null
    open_count: 0
    videos: []
  - course: '[[Planning and Implementing a Rapid Cloud Migration (RCM)]]'
    last_opened: null
    open_count: 0
    videos: []
created: '2026-05-03'
---

> <!-- lle:github-source --> [View this note on GitHub](https://github.com/thisis-romar/linkedin-learning-vault/blob/github-browse/Paths/Cloud%20Computing/Learning%20Paths/Introduction%20to%20Enterprise%20Cloud%20Migration%20Planning.md)

# Introduction to Enterprise Cloud Migration Planning

> [LinkedIn Learning](https://www.linkedin.com/learning/paths/introduction-to-enterprise-cloud-migration-planning) | 4 courses | Cloud Computing, DevOps, Software Development

## Courses

1. [[Cloud Concepts Planning An Application Migration Strategy]]
2. [[Design a Cloud Migration Strategy]] _(2h 21m)_
3. [[Dependency Mapping for Cloud Migration]] _(1h 35m)_
4. [[Planning and Implementing a Rapid Cloud Migration (RCM)]] _(1h 2m)_

## Course Flow

1. [[Cloud Concepts Planning An Application Migration Strategy]] → 2. [[Design a Cloud Migration Strategy]] → 3. [[Dependency Mapping for Cloud Migration]] → 4. [[Planning and Implementing a Rapid Cloud Migration (RCM)]]

## Topics

- [[Cloud Computing]]
- [[DevOps]]
- [[Software Development]]

## Related Paths

_Paths that share courses with this one:_

- [[Design and Present Your Cloud Strategy]] (2 shared courses)

---

Part of [[000 Map of Content]] · [[Cloud Computing]]